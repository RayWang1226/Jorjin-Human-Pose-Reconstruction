...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, required=True,
                        help="Where to save the SMPL-X parameters and output video")
    parser.add_argument("--save_video", action="store_true",
                        help="Save the rendered video output")
    parser.add_argument("--save_overlay", action="store_true",
                        help="Save overlay video from original video")
    parser.add_argument("--consecutive_frames", type=int, default=5,
                        help="Number of consecutive frames required to confirm a pose (default: 5)")
    ...
    out_video = None
    output_video_path = os.path.join(args.output_dir, "output_rendered.mp4")
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    overlay_path = os.path.join(args.output_dir, "output_pose_overlay.mp4")
    overlay_video = cv2.VideoWriter(
        overlay_path, fourcc, fps, (frame_width, frame_height))
    ...
    with tqdm(total=total_frames) as pbar:
        while cap.isOpened():
            ...
            # Track which hands were detected in this frame
            ...
            # Apply the corresponding SMPLX hand pose based on confirmed detections
            # Switch left and right due to mediapipe settings
            # Right hand
            confirmed_rpose = hand_pose_tracking["Left"]["persistent_pose"]
            if confirmed_rpose == "thumbs_up":
                current_rhand_pose = thumbs_up_rhand_pose
            elif confirmed_rpose == "pointing":
                current_rhand_pose = pointing_rhand_pose
            elif confirmed_rpose == "yeah":
                current_rhand_pose = yeah_rhand_pose
            elif confirmed_rpose == "ok":
                current_rhand_pose = ok_rhand_pose
            elif confirmed_rpose == "fist":
                current_rhand_pose = fist_rhand_pose
            else:  # Unknown or not detected
                current_rhand_pose = default_rhand_pose.copy()

            # Left hand
            confirmed_lpose = hand_pose_tracking["Right"]["persistent_pose"]
            if confirmed_lpose == "thumbs_up":
                current_lhand_pose = thumbs_up_lhand_pose
            elif confirmed_lpose == "pointing":
                current_lhand_pose = pointing_lhand_pose
            elif confirmed_lpose == "yeah":
                current_lhand_pose = yeah_lhand_pose
            elif confirmed_lpose == "ok":
                current_lhand_pose = ok_lhand_pose
            elif confirmed_lpose == "fist":
                current_lhand_pose = fist_lhand_pose
            else:  # Unknown or not detected
                current_lhand_pose = default_lhand_pose.copy()

            # Run ROMP for body pose estimation
            ...
            if result["body_pose"].shape[0] > 0:
                ...
                # Create SMPLX data with our detected hand poses
                smplx_data = {
                ...
                out_json_path = os.path.join(
                    args.output_dir, f"smplx_param_{frame_idx:05d}.json")
                with open(out_json_path, "w") as f:
                    json.dump(smplx_data, f)

                # Save visualization
                if args.save_video and "rendered_image" in result:
                    vis_frame = result["rendered_image"]

                    # Normalize and convert to uint8 if necessary
                    if vis_frame.dtype == np.float32 or vis_frame.max() <= 1.0:
                        vis_frame = (vis_frame * 255).clip(0,
                                                           255).astype(np.uint8)
            ...
            if args.save_overlay:
                 overlay_video.write(display_frame)
    ...
    print(
        f"[DONE] All frames processed and parameters saved to {args.output_dir}")
